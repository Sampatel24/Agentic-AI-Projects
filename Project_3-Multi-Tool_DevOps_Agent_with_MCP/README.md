...
I've built a Docker troubleshooter in Project 2. Now you add Kubernetes tools to the same agent, and learn how MCP lets any AI system use your tools — not just LangChain.
...
We added 3 Kubernetes tools to the Project 2 agents:
...
```bash
kubectl apply -f Project_3-Multi-Tool_DevOps_Agent_with_MCP/broken_pod.yaml
```
...
```bash
python3 Project_3-Multi-Tool_DevOps_Agent_with_MCP/agent.py
```
...
```bash
kubectl delete -f Project_3-Multi-Tool_DevOps_Agent_with_MCP/broken_pod.yaml
...
```json
{
  "mcpServers": {
    "k8s-tools": {
      "command": "/absolute/path/to/Agentic-AI-Projects/.venv/bin/python3",
      "args": ["/absolute/path/to/Agentic-AI-Projects/Project_3-Multi-Tool_DevOps_Agent_with_MCP/mcp_server.py"]
    }
  }
}
```

Get your repo path with `pwd` in the repo root, then append `.venv/bin/python3` for the command and `Project_3-Multi-Tool_DevOps_Agent_with_MCP/mcp_server.py` for the arg.
...
- Test manually: run `.venv/bin/python3 Project_3-Multi-Tool_DevOps_Agent_with_MCP/mcp_server.py` — if it crashes, you'll see the error.
...
Next: **[AIOps Demystified](../AIOps_Demystified/)**
